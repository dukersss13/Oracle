...
class Oracle:
    ...
    def set_oracle_config(self, oracle_config: dict):
        """
        """
        self.holdout = oracle_config["holdout"]
        self.last_x_games = oracle_config["last_x_games"]
    ...
    def pause_to_set_active_players(self):
        """
        """
        print("Set active players in active_players.json.")
    ...
    def get_testing_data(self, player_game_logs: np.ndarray, team: Team) -> tuple:
        """
        """
        #TODO: Fix rest days
        home_or_away = np.array([1., 0.]) if team == Team.HOME else np.array([0., 1.])
        x_test = np.mean(player_game_logs[self.holdout: self.holdout + self.last_x_games, :-4], axis=0)
        # x_test_categorical = 
        if self.holdout == 0:
            y_test = 0
        else:
    ...
    def get_players_forecast(self, filtered_players_logs: np.ndarray, team: Team) -> int:
        """
        """
        x_train, y_train = self.prepare_training_data(filtered_players_logs)
        x_test, actual_points = self.get_testing_data(filtered_players_logs, team)
    ...
    def get_team_forecast(self, team: Team):
        ...
        for players_name in data.active_players:
            ...
            if filtered_players_logs is None:
                ...
                forecasted_points, actual_points = self.get_players_forecast(filtered_players_logs, team)
